...
def inference(Demodel, img_batch_1, img_batch_2, inter_frames):
        ...
        for _ in range(len(remains)):
            ...
            dt_tensor = torch.ones((1, 1, int(x0.shape[2]//2), int(x0.shape[3]//2))).cuda() * dt
            with torch.no_grad():
                prediction = Demodel([x0, x1, dt_tensor])
...
class cnnModel(nn.Module):
    def __init__(self,color_channels=1,units=1,dim=64,frames=8):
        ...
        GenerateModule.load_state_dict(torch.load('./models/EDSC_m.ckpt', map_location=lambda storage, loc: storage)['model_state'])
        GenerateModule.eval()
        self.EDSC = GenerateModule.cuda()
    ...
    def forward(self, y, args):
        ...
        de_meas1 = inference(self.EDSC,y1.repeat(1,3,1,1),y2.repeat(1,3,1,1),5)
        de_meas1 = [torch.sum(de_meas1[k],dim=1,keepdim=True).cuda() for k in range(len(de_meas1))]
        de_meas1 = torch.stack(de_meas1,dim=1)
        de_meas1 = de_meas1[:,:,0]


        de_meas2 = inference(self.EDSC,y2.repeat(1,3,1,1),y3.repeat(1,3,1,1),8)
