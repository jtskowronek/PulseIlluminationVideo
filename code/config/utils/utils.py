...
def save_image(out,gt,mask,image_name,show_flag=False):
    if len(out.shape)==4:
        out = einops.rearrange(out,"c f h w->h (f w) c")
        gt = einops.rearrange(gt,"c f h w->h (f w) c")
        mask = einops.rearrange(mask,"c f h w->h (f w) c")
        result_img = np.concatenate([out,gt,mask],axis=0)
        result_img = result_img[:,:,::-1]
    else:
        out = einops.rearrange(out,"f h w->h (f w)")
        gt = einops.rearrange(gt,"f h w->h (f w)")
        mask = einops.rearrange(mask,"f h w->h (f w)")
        result_img = np.concatenate([out,gt,mask],axis=0)
