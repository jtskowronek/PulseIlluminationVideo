...
for it in range(args.iter):
   ...
   if it % args.saveEach==0:
      save2Mat(datacube,meas,measR,gt,iter_loss,args,it)
   ...
   if it > 1 and Loss.detach().cpu().numpy() < loss_b:
       ...
       datacube_b = datacube
       loss_b = Loss.detach().cpu().numpy()
       save2Mat(datacube_b,meas_b,measR_b,gt,iter_loss,args,"best")
       print("New Best!")
       


   print("===> Iter: %d - Total Loss: %.6f = %.2E + %.2E + %.2E + %.2E" % (it,
...
save2Mat(datacube,meas,measR,gt,iter_loss,args,"last")
